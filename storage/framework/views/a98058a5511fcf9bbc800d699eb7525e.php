...
<?php $__env->startSection('title', 'Équipe - RAIL Bénin'); ?>
<?php $__env->startSection('meta_description', 'Découvrez l\'équipe du RAIL - Réseau d\'appui aux initiatives locales au Bénin.'); ?>
...
<?php if($boardMembers && $boardMembers->count() > 0): ?>
        ...
        <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6 lg:gap-8 anim-stagger">
            ...
            <article class="group relative rounded-2xl overflow-hidden bg-white border border-ink-100/50 shadow-sm hover:shadow-xl transition-all duration-500 hover:-translate-y-1">
                <div class="relative aspect-[4/5] overflow-hidden">
                    <?php if($member->photo): ?>
                    <img src="<?php echo e(asset($member->photo)); ?>" alt="<?php echo e($member->name); ?>" class="absolute inset-0 w-full h-full object-cover transition-transform duration-700 group-hover:scale-105">
                    <?php else: ?>
                    <div class="absolute inset-0 w-full h-full flex items-center justify-center bg-gradient-to-br from-primary-400 to-accent-400 text-white text-6xl font-bold font-display"><?php echo e(substr($member->name, 0, 1)); ?></div>
                    <?php endif; ?>
                    <div class="absolute inset-0 bg-gradient-to-t from-black/85 via-black/25 to-transparent"></div>
                    <span class="absolute top-5 right-6 font-display font-bold text-5xl text-white/20 select-none"><?php echo e($loop->iteration < 10 ? '0' . $loop->iteration : $loop->iteration); ?></span>
                    <div class="absolute bottom-0 left-0 right-0 p-5 lg:p-6">
                        <h3 class="font-display font-bold text-white text-xl lg:text-2xl leading-tight"><?php echo e($member->name); ?></h3>
                        <p class="text-accent-300 text-[0.625rem] font-bold tracking-[0.2em] uppercase mt-1.5"><?php echo e($member->position); ?></p>
                    </div>
                </div>
                <?php if($member->bio): ?>
                <div class="p-5 lg:p-6 border-t border-ink-100/50">
                    <p class="text-sm text-ink-500 leading-relaxed"><?php echo e($member->bio); ?></p>
                </div>
                <?php endif; ?>
            </article>
...
<?php if($executiveMembers && $executiveMembers->count() > 0): ?>
<section class="relative overflow-hidden py-20 lg:py-28">
    ...
    <div class="container relative z-10">
        ...
        <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6 anim-stagger">
            <?php $__currentLoopData = $executiveMembers; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $member): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                ...
                <div class="relative w-28 h-28 mx-auto mb-6">
                    <div class="absolute inset-0 rounded-full bg-gradient-to-br from-accent-400 to-primary-500 p-[3px] group-hover:scale-105 transition-transform duration-500">
                        <div class="w-full h-full rounded-full overflow-hidden bg-primary-950">
                            ...
                            <?php else: ?>
                            <div class="w-full h-full flex items-center justify-center text-white text-2xl font-bold font-display"><?php echo e(substr($member->name, 0, 1)); ?></div>
                            <?php endif; ?>
                        </div>
                    </div>
                </div>
                <h3 class="font-display font-bold text-white text-lg leading-snug"><?php echo e($member->name); ?></h3>
                <div class="w-8 h-[2px] bg-gradient-to-r from-accent-400 to-accent-500 rounded-full mx-auto my-3.5"></div>
                <p class="text-accent-300 text-[0.625rem] font-bold tracking-[0.2em] uppercase"><?php echo e($member->position); ?></p>
                <?php if($member->bio): ?>
                <p class="text-white/50 text-sm leading-relaxed mt-3"><?php echo e($member->bio); ?></p>
                <?php endif; ?>
            </article>
            <?php endforeach; $__env->popLoop(); $loop = $__env->getLastLoop(); ?>
        </div>
...
<section class="section">
    <div class="container">
        <div class="cta-section animate-fade-up">
            ...
            <a href="<?php echo e(route('contact')); ?>" class="btn btn-white btn-lg">
...
<?php echo $__env->make('layouts.app', array_diff_key(get_defined_vars(), ['__data' => 1, '__path' => 1]))->render(); ?><?php /**PATH C:\xampp\htdocs\ongRail\resources\views/pages/equipe.blade.php ENDPATH**/ ?>